 ...
 *
 * Pipeline: nonce -> honeypot/time-trap -> rate limit -> Turnstile ->
 * sanitize + validate -> store lead -> notify email -> webhook -> redirect.
 *
 * @package PetitForm
 */
...
function petit_form_handle_submit() {
	...
	$traps = petit_form_verify_traps( $_POST, $form_id ); // phpcs:ignore WordPress.Security.NonceVerification -- nonce verified above
	...
	foreach ( $fields as $field ) {
		$raw    = isset( $_POST[ 'pf_f_' . $field['key'] ] ) ? wp_unslash( $_POST[ 'pf_f_' . $field['key'] ] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification
